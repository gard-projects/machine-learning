...
If we take a look at the dataset for the training set ("l1_train.csv") we note the following: \
$n =  696 - 1 = 695 \quad$ (we decrease by one to exclude the header label) \
$m = 2 - 1 = 1 \quad$ (there is only one dependent variable, y) 

As a challenge for the reader, try to calculate the respective size (m and n) for the testing set.
...
```
> [!NOTE]
> The X_features matrix will be covered shortly, the example above is meant to show how you can get shape of any np.ndarray object.
...
```
> [!IMPORTANT]
> **NB!** If you are running this project locally, the location where you the CSV files are stored may be different. In that case just replace the location in the `read_csv(...)` with your respective path.

# Creating the model structure
> [!IMPORTANT]
> I assume you have some grasp of classes in Python, as it will not be covered.

In order to make the implementation of linear regression easier, I make use of Python classes. The constructor takes in two arguments, **epochs** and **learning_rate $\alpha$**. The cost history field is used to keep track of the loss value during each epoch.
...
```
Now that we have the fundementals down, what is needed for creating a linear regression model? 
...
## Fitting the data
As described earlier, we use the `fit(self, X_features: np.ndarray, y: np.ndarray)` function as our main method of running the model. We will now make use of **n** and **m** which we found earlier. \
> **X_features**: a matrix of shape $n \times m$, represents all of our samples (i.e. the values of our predictors)
> 
> **y**: a vector of shape $n \times 1$, representing the actual responses (the values of the dependent variable)
...
> [!TIP]
> Try understand why this is the case, as this will help in understanding the equations that come later (e.g. for **hypothesis function $h_{\theta}(x)$** and **gradient vector $\nabla_{\theta}J(\theta)$**)
...
```
It is important than the variable **m** is included in the slicing. If omitted the result of the slicing operation will not be of the correct shape expected by the `fit` function. \
...
To estimate the responses we use a **hypothesis function $h_{\theta}(x)$, also called a regression function. The generalized regression function takes in single predictor, thus a 1D numpy array of some length m (m independent variables). 
...
We can optimize this equation by using matrices. We will follow the convention and call the cost function $J(\theta)$, thus
...
> The reason for this behaviour is due to **duality** between dot product and matrix multiplication. The result of this matrix multiplication is a $1x1$ matrix, which can be interpreted as a scalar.
...
Think of the current position as a ball. The random intialization can be thought of as a person holding the ball in some random location on the graph, which in this case is one dimensional (1D).
...
We use the gradient $\textemdash$ which tells us the direction of steepest descent **in the current position** $\textemdash$ to move in the 
direction which leads us to a minimum of the loss function. The arrows in the image represents the location of the ball at each epoch, which is controlled by the **learning rate $\alpha$**. So in each **epoch** the ball recieves a new position, i.e. the weights' values are slightly modified. The ideal situation is to end up in a global minimum (indicated by the picture), not a local minimum. When the ball rests in a minimum, the corresponding coordinates of the ball will reflect the values of the weights in the network.
...
>[!NOTE]
> The data we are using is 1D, since we have one predictor only. For higher dimensional data plotting becomes harder, therefore you could instead use metrics to check the model's performance, like coefficient of determination, $R^2$.
